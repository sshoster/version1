...
- [x] Chat-app conversation: internal scroll pane sized to content, WhatsApp-style bubbles with per-author accent colors, floating jump-to-latest chip, instant (non-fighting) auto-follow, desktop sticky composer.
- [x] Super-admin dashboard (`/admin`, `SUPER_ADMIN_EMAILS` allowlist, 404-hidden): user search/create/edit/password-reset/suspend/reactivate; SOFT delete with identity revival on re-registration (same user id, meetings restored, Google revives too); room list with copyable join code + join link (`/?code=` prefills the home page) and full cascade purge.
- [x] Platform golden rules: `platform_settings` document + `@Primary` LLM decorator injecting the rules into every system prompt; admin-page editor; defaults cover respectful language, reject/re-ask unclear answers, ask-once, stay-on-topic, injection immunity.
- [x] Assistant UX: re-run a round from PROPOSAL_READY; pending private answers named (✍️ participants badge + named waiting text; IDs only, never question content); questions carry 2–4 one-tap suggested answers (validated in the turn schema).
- [x] Discussion documents: share summary/understandings into the conversation; email the agreement draft as an attached file to all/selected participants (EmailSender attachments for SMTP + Brevo); errors render next to the button that caused them.
- [x] Invite/join identity reconciliation: email dedup on invite, member-email invites refused, joining by any route clears the person's other pending entries, stale pendings hidden; accept page shows the signed-in account with account switching; join requests name the deciding admin (+email) and decisions email the requester.
- [x] App shell: room tools (invite/files/timeline) moved into the drawer as slide-in panels; the drawer lists the user's discussions (collapsible, styled toggle); shared `RoomsStore` client cache (60s stale-while-revalidate) for home + drawer; async `EmailDispatcher` on a dedicated executor for all outbound mail; room admins can delete CLOSED discussions (cascade purge).
